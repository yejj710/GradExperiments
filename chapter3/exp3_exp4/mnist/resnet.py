...
def _train_steps(type, devices, server, x_train, y_train, x_test, y_test, epochs=10, callback=None):
    # num_classes = 10
    input_shape = (28, 28, 1)
    model = create_model(input_shape=input_shape, num_classes=10, name="resnet", dataset="mnist")
    ...
    file_path = os.path.join(save_directory, f'../evaluate_metrics/resnet_{type}_226.pkl')
...
if __name__ == "__main__":
    ...
    # ['plain', 'ldp', 'phe', 'ss']
    for agg_type in ['ldp', 'ldp', 'phe', 'ss']:
